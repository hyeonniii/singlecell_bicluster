...
/* make E_ij function*/
sp_mat makeE1(int idx,int n, int p, arma::mat n3){
  int i=n3(idx,0);
  int j=n3(idx,1);
...
/* make E_kl function*/
sp_mat makeE2(int idx, int n, int p, arma::mat p3){
  int i=p3(idx,0);
  int j=p3(idx,1);
...
// [[Rcpp::export]]
List sc_bicluster(double lambda, double gamma1, double del, arma::mat Z, arma::mat X,double tol_error, double max_iter){
  ...
  arma::mat mat1_1(n,n);
  mat1_1.fill(-1);
  arma::mat mat1_2(n,n,fill::eye);
  arma::mat diag1=n*mat1_2+mat1_1;
  
  
  for (int i=0;i<p;i++){
    
    summat1.submat(n*i,n*i,n*i+n-1,n*i+n-1)=diag1;
  }
  ...
  arma::mat mat2(n*p,n*p,fill::eye);  
  summat2=(p)*mat2+symmatu(summat2);
  ...
  arma::mat inv_mat1=id+2*gamma1*trans(idxom)*idxom+del*summat1+del*summat2;
  arma::mat inv_mat=as<arma::mat>(wrap(f(Named("x")=inv_mat1)));
  ...
  for(int iter=0;iter<max_iter;iter++){
    ...
    arma::vec A1(p);
    for(int i=0;i<m1;i++){
      a_ij=makeE1(i,n,p,n3)*vecx_new-(1/del)*u_old.col(i);
    ...
    arma::vec B1(n);
    for(int i=0;i<m2;i++){
      b_ij=makeE2(i,n,p,p3)*vecx_new-(1/del)*v_old.col(i);
    ...
    arma::mat v_new(n,m2);
    for(int i=0;i<m1;i++){
      u_new.col(i)=u_old.col(i)+del*(A_new.col(i)-makeE1(i,n,p,n3)*vecx_new);
    }
    ...
    Rcout<<"updating V\n";
    for(int i=0;i<m2;i++){
      v_new.col(i)=v_old.col(i)+del*(B_new.col(i)-makeE2(i,n,p,p3)*vecx_new);
    }
